...
listTable.edit = function(obj, act, id)
{
  ...
  var val = Browser.isIE ? Utils.trim(obj.innerText) : Utils.trim(obj.textContent);

  /* 创建一个输入框 */
  var txt = document.createElement("INPUT");
  txt.value = (val == 'N/A') ? '' : val;
  txt.style.width = (obj.offsetWidth + 12) + "px" ;
  txt.style.background="#fbdadc";
...
listTable.toggle = function(obj, act, id)
{
  ...
  if (res.error == 0)
  {
    obj.src = (val == 1 ? '../Images/yes.gif' : '../Images/no.gif');
    //obj.src = obj.src == "../Images/no.gif" ? '../Images/yes.gif' : '../Images/no.gif';
  }
}
...
listTable.remove = function(id, cfm, opt)
{
  ...
  if (confirm(cfm))
  {
    var args = "act=" + opt + "&val=&id=" + id + this.compileFilter();
