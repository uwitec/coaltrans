    ...
    public void ProcessRequest (HttpContext context) 
    {
        ...
        foreach (XmlNode cluster in clusters)
        {
            ...
            if (radius > 6f)
            {
                ...
                nodeOneLeft = (int)Math.Ceiling(int.Parse(cluster.SelectSingleNode("autn:x1", nsmgr).InnerText) * 0.68) + 165;
                nodeOneTop = (int)Math.Ceiling(int.Parse(cluster.SelectSingleNode("autn:y1", nsmgr).InnerText) * 0.68) + 250;
                nodeTwoLeft = (int)Math.Ceiling(int.Parse(cluster.SelectSingleNode("autn:x2", nsmgr).InnerText) * 0.68) + 165;
                ...
                html.AppendFormat("<div class=\"node\" id=\"{0}\" style=\"border: 1px solid rgb(255, 0, 0); position: absolute; z-index: 2; font-size: 1px; width: {3}px; height: 4px; visibility: visible; top: {1}px; left: {2}px; cursor:pointer;\" ></div>", nodeId, nodeOneTop, nodeOneLeft, width);
